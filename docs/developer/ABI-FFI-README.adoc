...
│       └── include/
│           └── spline.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── spline.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/spline.h
```
...
int main() {
    void* handle = spline_init();
    if (!handle) return 1;

    int result = spline_process(handle, 42);
    if (result != 0) {
        const char* err = spline_last_error();
...
Compile with:
```bash
gcc -o example example.c -lspline -L./zig-out/lib
```

### From Idris2

```idris
import SPLINE.ABI.Foreign
...
#[link(name = "spline")]
extern "C" {
    fn spline_init() -> *mut std::ffi::c_void;
    fn spline_free(handle: *mut std::ffi::c_void);
    fn spline_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = spline_init();
        assert!(!handle.is_null());

        let result = spline_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libspline = "libspline"

function init()
    handle = ccall((:spline_init, libspline), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:spline_process, libspline), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:spline_free, libspline), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/spline.h
   ```
