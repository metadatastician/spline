...
│       └── include/
│           └── weft.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── weft.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/weft.h
```
...
int main() {
    void* handle = weft_init();
    if (!handle) return 1;

    int result = weft_process(handle, 42);
    if (result != 0) {
        const char* err = weft_last_error();
...
Compile with:
```bash
gcc -o example example.c -lweft -L./zig-out/lib
```

### From Idris2

```idris
import WEFT.ABI.Foreign
...
#[link(name = "weft")]
extern "C" {
    fn weft_init() -> *mut std::ffi::c_void;
    fn weft_free(handle: *mut std::ffi::c_void);
    fn weft_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = weft_init();
        assert!(!handle.is_null());

        let result = weft_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libweft = "libweft"

function init()
    handle = ccall((:weft_init, libweft), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:weft_process, libweft), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:weft_free, libweft), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/weft.h
   ```
