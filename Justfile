...
cookbook:
    ...
    echo "= spline Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/spline.1 << EOF
    .TH spline 1 "$(date +%Y-%m-%d)" "{{version}}" "spline Manual"
    .SH NAME
    spline \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/spline.1"
...
container-init:
    ...
    read -rp "Service name (e.g. my-api) [spline]: " _SERVICE_NAME
    SERVICE_NAME="${_SERVICE_NAME:-spline}"
...
container-build *args:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman build -t spline:latest -f container/Containerfile .
    elif [ -f "build/Containerfile" ]; then
        podman build -t spline:latest -f build/Containerfile .
    elif [ -f "Containerfile" ]; then
        podman build -t spline:latest -f Containerfile .
    else
...
container-push:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman push spline:latest
    fi

# Run container interactively (for debugging)
container-run *args:
    podman run --rm -it spline:latest {{args}}
